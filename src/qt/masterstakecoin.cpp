...
{
    return QCoreApplication::translate("masterstakecoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in masterstakecoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in masterstakecoin.qrc)
...
/** Class encapsulating MasterStakeCoin Core startup and shutdown.
...
/** Main MasterStakeCoin application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // masterstakecoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. MasterStakeCoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(masterstakecoin);
    Q_INIT_RESOURCE(masterstakecoin_locale);
    ...
    /// 6. Determine availability of data directory and parse masterstakecoin.conf
    ...
    // masterstakecoin: links repeatedly have their payment requests routed to this process:
