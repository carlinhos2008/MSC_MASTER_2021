...
const char * const BITCOIN_CONF_FILENAME = "masterstakecoin.conf";
const char * const BITCOIN_PID_FILENAME = "masterstakecoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "masterstakecoin" is a composite category enabling all MasterStakeCoin-related debug output
                if(ptrCategory->count(std::string("masterstakecoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MasterStakeCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MasterStakeCoin
    // Mac: ~/Library/Application Support/MasterStakeCoin
    // Unix: ~/.masterstakecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MasterStakeCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MasterStakeCoin";
#else
    // Unix
    return pathRet / ".masterstakecoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty masterstakecoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override masterstakecoin.conf
