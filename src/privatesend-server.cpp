...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all MasterStakeCoin related functionality
...
/*
    ...
    allow endless transaction that would bloat MasterStakeCoin and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all MasterStakeCoin specific functionality
