...
export BUILDER_IMAGE_NAME="masterstakecoin-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/masterstakecoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_MSC_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
