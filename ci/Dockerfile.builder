...
# masterstakecoin_hash
RUN git clone https://github.com/masterstakecoinpay/masterstakecoin_hash
RUN cd masterstakecoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} masterstakecoin
RUN useradd -u ${USER_ID} -g masterstakecoin -s /bin/bash -m -d /masterstakecoin masterstakecoin
...
RUN mkdir /masterstakecoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /masterstakecoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /masterstakecoin-src

USER masterstakecoin
